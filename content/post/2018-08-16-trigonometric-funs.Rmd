...
Un detalle interesante es como ggplot hace la leyenda. En `stat_function` especifico `aes(colour = "sin(x)")`. Eso hace que el color de ese geom quede ligado al texto `"sin(x)"` en la leyenda.

Otra cosa que resultó más complicada de lo esperado son las etiquetas del eje x con letras griegas. Este post de stackoverflow sobre [como poner letras griegas en ggplot]
(https://stackoverflow.com/questions/5293715/how-to-use-greek-symbols-in-ggplot2) fue de bastante utilidad, pero me parece que la solución que encontré es demasiado *verbose*.
